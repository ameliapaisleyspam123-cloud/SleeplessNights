...
function LayoutInner() {
  ...
  return (
    <div className="min-h-screen parchment flex">
      <aside
        className={`hidden lg:flex shrink-0 flex-col border-r border-border/60 bg-card/40 backdrop-blur-sm transition-all duration-300 sticky top-0 h-screen ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between px-4 h-14 border-b border-border/60 bg-background/90 backdrop-blur">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-sm bg-primary text-primary-foreground flex items-center justify-center font-display text-lg">⚔</div>
          <span className="font-display text-lg tracking-wide truncate">{campaign?.name || "The Grimoire"}</span>
        </div>
...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className={`flex items-center border-b border-border/60 h-16 shrink-0 ${collapsed ? "justify-center px-2" : "justify-between px-4"}`}>
        {!collapsed && (
        ...
        <button
          ...
          className="text-muted-foreground hover:text-foreground transition-colors hidden lg:flex shrink-0 ml-1"
      ...
      {!collapsed && (
        <div
          className={`mx-3 mt-3 mb-1 px-3 py-2 rounded-sm flex items-center gap-2 shrink-0 ${
            isAdmin ? "bg-accent/10 border border-accent/30" : "bg-secondary border border-border"
          }`}
        >
      ...
      <div className={`border-t border-border/60 p-3 shrink-0 ${collapsed ? "flex flex-col items-center gap-2" : ""}`}>
        ...
        {isAdmin && (
          <button
            ...
            className={`flex items-center gap-2 text-xs text-muted-foreground hover:text-accent transition-colors mb-1 px-1 w-full ${collapsed ? "justify-center" : ""}`}
        ...
        <button
          ...
          className={`flex items-center gap-2 text-xs transition-colors mb-1 px-1 w-full ${collapsed ? "justify-center" : ""} ${
            splitOpen ? "text-accent" : "text-muted-foreground hover:text-accent"
          }`}
          title="Toggle Initiative Split-Screen"
        >
        ...
        <Link
          to="/campaign"
          className={`flex items-center gap-2 text-xs text-muted-foreground hover:text-accent transition-colors mb-1 px-1 ${collapsed ? "justify-center" : ""}`}
        ...
        {isSuperuser && (
          <button
            onClick={onToggleDmOverride}
            className={`flex items-center gap-2 text-xs transition-colors mb-1 px-1 w-full ${collapsed ? "justify-center" : ""} ${
              dmOverride ? "text-accent" : "text-muted-foreground hover:text-accent"
            }`}
            title="Toggle DM View"
          >
            <Shield className="w-3.5 h-3.5 shrink-0" />
            {!collapsed && <span>{dmOverride ? "DM View: On" : "DM View: Off"}</span>}
          </button>
        )}
        <Button variant="ghost" size={collapsed ? "icon" : "sm"} className={`text-muted-foreground ${collapsed ? "" : "w-full justify-start"}`} onClick={() => base44.auth.logout()} title="Depart">
